...
public class MarqueeLabel: UILabel {
    ...
    @IBInspectable public var fadeLength: CGFloat = 0.0 {
        didSet {
            if fadeLength != oldValue {
                applyGradientMask(fadeLength: fadeLength, animated: true)
                updateAndScroll()
            }
        }
    }
    ...
    class func restartLabelsOfController(controller: UIViewController) {
        MarqueeLabel.notifyController(controller: controller, message: .Restart)
    }
    ...
    class func controllerViewWillAppear(controller: UIViewController) {
        MarqueeLabel.restartLabelsOfController(controller: controller)
    }
    ...
    private func setup() {
        ...
        NotificationCenter.defaultCenter().addObserver(self, selector: #selector(MarqueeLabel.restartForViewController(_:)), name: MarqueeKeys.Restart.rawValue, object: nil)
        NotificationCenter.defaultCenter().addObserver(self, selector: #selector(MarqueeLabel.labelizeForController(_:)), name: MarqueeKeys.Labelize.rawValue, object: nil)
        NotificationCenter.defaultCenter().addObserver(self, selector: #selector(MarqueeLabel.animateForController(_:)), name: MarqueeKeys.Animate.rawValue, object: nil)
        // UIApplication state notifications
        NotificationCenter.defaultCenter().addObserver(self, selector: #selector(MarqueeLabel.restartLabel), name: UIApplicationDidBecomeActiveNotification, object: nil)
        NotificationCenter.defaultCenter().addObserver(self, selector: #selector(MarqueeLabel.shutdownLabel), name: UIApplicationDidEnterBackgroundNotification, object: nil)
    }
...
private struct Scroller {
    ...
    init(generator gen: @escaping (interval: CGFloat, _ delay: CGFloat) -> [Scroll]) {
        self.generator = gen
    }
